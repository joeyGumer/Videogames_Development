...
//EXERCISE 1
bool j1FileSystem::CreateDir(const char* dir)
{
	bool ret = false;

	if (Exists(dir) == false || IsDirectory(dir) == false)
	{
		...
		if (!ret)
			...
			LOG("Directory %s created succesfully\n", dir);
	}
	else
	{
		LOG("File System error while creating directory %s: this directory already exists\n", dir);
	}
