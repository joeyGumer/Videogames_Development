...
{
	pugi::xml_node& gui_element = node.child("GuiElement");
	for (int i = 0; gui_element; gui_element = gui_element.next_sibling("GuiElement"))
	{
		for (p2List_item<GuiElement*>* item = gui_elements.start; item; item = item->next)
		{
			if (item->data->draggable)
			{
				if (gui_element.attribute("index").as_int() == i)
				{
					item->data->SetLocalPosition({ gui_element.attribute("x").as_int(), gui_element.attribute("y").as_int() });
				}
				else
					i++;
			}
...
{
	p2List_item<GuiElement*>* item = gui_elements.start;
	for (int i = 0; item; item = item->next)
	{
		if (item->data->draggable)
		{
			pugi::xml_node gui_element = node.append_child("GuiElement");
		
			gui_element.append_attribute("x") = item->data->GetLocalPosition().x;
			gui_element.append_attribute("y") = item->data->GetLocalPosition().y;	
			gui_element.append_attribute("index") = i;
			i++;
		}
	}
