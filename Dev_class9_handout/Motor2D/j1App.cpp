...
j1App::j1App(int argc, char* args[]) : argc(argc), args(args)
{
	...
	total_time = AppTimer.ReadMs() / 1000;
	amount_frames = 0;
	last_fps = 0.0f;
	last_fps_tmp = 0.0f;
	frame_seconds = 0.0f;
...
void j1App::PrepareUpdate()
{
	...
	timestep = AppTimer.ReadMs() / 1000 - frame_seconds;
	if (SecondTimer.ReadMs()/1000 < 1.0f)
	{
		last_fps++;
	}
...
void j1App::FinishUpdate()
{
	...
	float seconds_since_startup = AppTimer.ReadMs()/1000;
	float dt = timestep;
	if (dt > 1)
		last_fps_tmp = last_fps;
	uint32 last_frame_ms = ModuleTimer.Read();
	uint32 frames_on_last_update = 0;
	if (SecondTimer.ReadMs() / 1000 >= 1)
	{
		last_fps_tmp = last_fps;
		last_fps = 0;
		SecondTimer.Start();
		...
		frames_on_last_update = last_fps_tmp;
